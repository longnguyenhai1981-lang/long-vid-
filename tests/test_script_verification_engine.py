...
def _valid_research_package(central_question: str, claims=None) -> ResearchPackage:
    return ResearchPackage(
        ...
        else [Claim(claim_id="C001", claim="Flutter is self-excited", status="SAFE", confidence="HIGH")],
    )
...
def _create_project_in_script_verification(engine, script_plan=None):
    ...
    return project_id, research_package, narrative_plan, packaging, script_plan
    ...
    return json.dumps(
        {
            ...
            "recommended_rewrites": recommended_rewrites or [],
        }
    )
...
def test_script_verification_success(engine):
    project_id, research_package, narrative_plan, packaging, script_plan = (
        _create_project_in_script_verification(engine)
    )
    provider = FakeLLMProvider([_response(_report_json())])

    sv_engine = ScriptVerificationEngine(engine, provider, _global_config(), _llm_settings())
    result = sv_engine.run(ScriptVerificationInput(project_id=project_id))

    assert provider.call_count == 1
    assert isinstance(result.report, ScriptVerificationReport)
    assert result.report.status == GateStatus.PASS
    ...
    # ScriptPlan must never be modified or re-saved by verification.
    ...
    assert runs[0].output_id is None
...
def test_script_verification_status_normalized_up_when_llm_understates(engine):
    project_id, *_ = _create_project_in_script_verification(engine)
    provider = FakeLLMProvider(
        [_response(_report_json(status="PASS", unsupported_lines=["L099: unsupported claim"]))]
    )
    ...
    assert provider.call_count == 1
    assert result.report.status == GateStatus.REFRAME
    assert result.report.unsupported_lines == ["L099: unsupported claim"]


def test_script_verification_status_normalized_down_when_llm_overstates(engine):
    project_id, *_ = _create_project_in_script_verification(engine)
    provider = FakeLLMProvider([_response(_report_json(status="REJECT"))])
...
def test_script_verification_severity_choice_kept_when_issues_present(engine):
    project_id, *_ = _create_project_in_script_verification(engine)
    provider = FakeLLMProvider(
        [_response(_report_json(status="REJECT", overstated_lines=["L001: overstated"]))]
    )

    sv_engine = ScriptVerificationEngine(engine, provider, _global_config(), _llm_settings())
    result = sv_engine.run(ScriptVerificationInput(project_id=project_id))

    assert result.report.status == GateStatus.REJECT


# ---------------------------------------------------------------------------
# LLM findings pass through unchanged (engine does not police semantic judgment)
# ---------------------------------------------------------------------------


def test_script_verification_reports_qualified_disputed_overstatement(engine):
    project_id, *_ = _create_project_in_script_verification(engine)
    provider = FakeLLMProvider(
        [
            _response(
                _report_json(
                    status="REFRAME",
                    overstated_lines=["L001: claims certainty the DISPUTED claim does not support"],
                )
            )
        ]
    )
    ...
    assert result.report.status == GateStatus.REFRAME
    assert result.report.overstated_lines == [
        "L001: claims certainty the DISPUTED claim does not support"
    ]


def test_script_verification_reports_dangerous_simplification(engine):
    project_id, *_ = _create_project_in_script_verification(engine)
    provider = FakeLLMProvider(
        [_response(_report_json(status="REJECT", dangerous_simplifications=["L001: reverses the causal mechanism"]))]
    )
    ...
    assert result.report.status == GateStatus.REJECT
    assert result.report.dangerous_simplifications == ["L001: reverses the causal mechanism"]


def test_script_verification_reports_packaging_promise_mismatch(engine):
    project_id, *_ = _create_project_in_script_verification(engine)
    provider = FakeLLMProvider(
        [
            _response(
                _report_json(
                    status="REFRAME",
                    overstated_lines=["L001: script never delivers the approved promise"],
                    recommended_rewrites=["L001: reconnect the ending to the approved promise"],
                )
            )
        ]
    )
    ...
    assert result.report.recommended_rewrites == [
        "L001: reconnect the ending to the approved promise"
    ]
...
def test_script_verification_structured_retry_then_success(engine):
    project_id, *_ = _create_project_in_script_verification(engine)
    invalid_json = '{"unsupported_lines": []}'
    provider = FakeLLMProvider([_response(invalid_json), _response(_report_json())])
...
# ---------------------------------------------------------------------------
# Business correction (deterministic claim-reference defense in depth)
# ---------------------------------------------------------------------------


def test_script_verification_business_correction_then_success(engine):
    bad_script_plan = _valid_script_plan(claim_ids=["C999"])
    project_id, *_ = _create_project_in_script_verification(engine, script_plan=bad_script_plan)

    not_acknowledged = _response(_report_json(status="PASS"))
    acknowledged = _response(
        _report_json(status="REFRAME", unsupported_lines=["L001: references an unknown claim"])
    )
    provider = FakeLLMProvider([not_acknowledged, acknowledged])
    ...
    assert "L001" in provider.received_requests[1].user_prompt
    assert result.report.status == GateStatus.REFRAME
    assert result.report.unsupported_lines == ["L001: references an unknown claim"]
...
# ---------------------------------------------------------------------------
# Business correction exhaustion
# ---------------------------------------------------------------------------


def test_script_verification_business_correction_exhaustion(engine):
    bad_script_plan = _valid_script_plan(claim_ids=["C999"])
    project_id, *_ = _create_project_in_script_verification(engine, script_plan=bad_script_plan)

    still_not_acknowledged = _response(_report_json(status="PASS"))
    still_wrong = _response(_report_json(status="PASS"))
    provider = FakeLLMProvider([still_not_acknowledged, still_wrong])

    sv_engine = ScriptVerificationEngine(engine, provider, _global_config(), _llm_settings())
    with pytest.raises(ScriptVerificationBusinessError):
        sv_engine.run(ScriptVerificationInput(project_id=project_id))
...
def test_script_verification_invalid_start_state(engine):
    project_id, *_ = _create_project_in_script_verification(engine)
    update_project_state(engine, project_id, ProjectState.SCRIPT)
    provider = FakeLLMProvider([])
...
# ---------------------------------------------------------------------------
# Missing upstream artifact (x4)
# ---------------------------------------------------------------------------


def test_script_verification_missing_research_package(engine):
...
def test_script_verification_missing_narrative_plan(engine):
    idea = _valid_idea_candidate()
    project_id = _new_project(engine)
    update_project_state(engine, project_id, ProjectState.IDEA_DISCOVERY)
    save_artifact(engine, project_id, IDEA_CANDIDATE_ARTIFACT_TYPE, idea)
...
def test_script_verification_missing_packaging_prototype(engine):
    idea = _valid_idea_candidate()
    project_id = _new_project(engine)
    update_project_state(engine, project_id, ProjectState.IDEA_DISCOVERY)
    save_artifact(engine, project_id, IDEA_CANDIDATE_ARTIFACT_TYPE, idea)
...
def test_script_verification_missing_script_plan(engine):
    project_id, idea, research_package, narrative_plan, packaging = _create_project_in_script(engine)
    update_project_state(engine, project_id, ProjectState.SCRIPT_VERIFICATION)
...
def test_script_verification_llm_failure_marks_module_run_failed(engine):
    project_id, *_ = _create_project_in_script_verification(engine)
...
def test_script_verification_structured_exhaustion(engine):
    project_id, *_ = _create_project_in_script_verification(engine)
    invalid_json = '{"unsupported_lines": []}'
    provider = FakeLLMProvider(
    ...
    sv_engine = ScriptVerificationEngine(
