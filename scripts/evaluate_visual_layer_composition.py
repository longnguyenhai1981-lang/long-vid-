"""Manual integration-evaluation utility for Phase 26: deterministic
visual layer composition.

Drives the real VisualRenderer.run() path end to end against a
disposable, four-beat VisualPlan, authored in DELIBERATELY SCRAMBLED
order -- V4 (COMPOSITION) first, V1 (GENERATED_STILL, its own
background) last:

    V4  COMPOSITION      (background=V1, overlay=V2, overlay=V3)
    V3  TI_STATE STANDALONE
    V2  DIAGRAM
    V1  GENERATED_STILL

so this script proves the renderer's dependency ordering is real -- V1
and V2 must render before V4 despite being authored after it -- and not
an accident of iterating visual_plan.beats in file order (the same
proof pattern scripts/evaluate_visual_renderer_beat_composition.py,
Phase 24, established).

No live image API call anywhere:

- V1's "provider" output is produced by FakeVisualProvider with a real,
  Pillow-decodable fixture image (queued exactly once).
- V2 is rendered by the real, local, deterministic DiagramRenderer
  (Phase 25) -- zero provider calls.
- V3 resolves the REAL active v1 canonical TiAssetSet (read from
  data/motily.db) via the real TiCompositor's retriever, STANDALONE mode
  -- zero provider calls, zero compositing (no background supplied for
  V3 itself; it is consumed as a composition OVERLAY instead).
- V4 combines V1/V2/V3's already-rendered outputs via the real
  VisualLayerCompositor (Phase 26) -- zero provider calls.

Total provider calls for the whole run: 1 (V1 only).
...
(data/visual_layer_composition_evaluation/eval.db, recreated fresh on
every run) -- never against the shared data/motily.db.

Usage:

    python scripts/evaluate_visual_layer_composition.py
"""
...
EVAL_DIR = Path("data/visual_layer_composition_evaluation")
...
def _fake_still_response() -> VisualRenderResponse:
    """A real, Pillow-decodable PNG -- V1's "provider" output, which both
    the DIAGRAM overlay and the standalone Tí overlay will be composited
    on top of by VisualLayerCompositor."""
...
def _diagram_spec() -> DiagramSpec:
    """A tiny deterministic diagram (Phase 25) -- a labeled circle -- for
    V2, rendered by the real local DiagramRenderer, never a provider."""
    return DiagramSpec(
        canvas=DiagramCanvas(width=300, height=200, background_color=None),  # transparent overlay
        elements=[
            DiagramEllipse(
...
def _build_four_beat_project_at_mvp_complete(engine) -> tuple[Project, ScriptPlan, VoicePlan, VisualPlan]:
    """The minimum project graph VisualRenderer.run() requires, with a
    four-beat VisualPlan authored in SCRAMBLED order: V4 (COMPOSITION),
    V3 (TI_STATE STANDALONE), V2 (DIAGRAM), V1 (GENERATED_STILL) -- the
    exact reverse of rendering order, deliberately (see this script's
    module docstring). Mirrors scripts/evaluate_visual_renderer_beat_
    composition.py's fixture builder, extended to four beats -- this is
    scaffolding to reach MVP_COMPLETE, not the thing being evaluated."""
    created = datetime.now(timezone.utc)
    project = Project(
        title_internal="Phase 26 visual layer composition evaluation",
        created_at=created,
        updated_at=created,
    ...
    # Authored order deliberately scrambled: V4, V3, V2, V1 -- the exact
    # reverse of rendering order. See this script's module docstring.
    visual_plan = VisualPlan(
        ...
        beats=[
            VisualBeat(
                ...
                complexity="C1",
                concept="Final composed frame: bridge background, diagram overlay, Tí overlay",
            ...
            VisualBeat(
                ...
                media_type="DIAGRAM",
                complexity="C1",
                concept="A simple labeled diagram overlay",
                primary_focus="The diagram callout",
...
def main() -> int:
    ...
    project, *_ = _build_four_beat_project_at_mvp_complete(eval_engine)

    # Exactly one queued response -- for V1. If any other beat triggered a
    # second provider call (a fallback bug), FakeVisualProvider would
    # raise "exhausted" instead of the renderer succeeding.
    ...
    composition_spec = CompositionSpec(
        layers=[
            ...
            OverlayLayerSource(
                id="ti_overlay", source_beat_id="V3", z_index=2,
                anchor=LayerAnchor.BOTTOM_RIGHT, scale=LayerScale(relative_height=0.45), margin=16,
            ),
        ]
    )
    renderer_input = VisualRendererInput(
        project_id=project.project_id,
        composition_specs={"V4": composition_spec},
    )

    print(f"Active TiAssetSet: {active_set.id} version {active_set.version!r}")
    print(f"Evaluation project: {project.project_id}")
    print("Authored VisualPlan beat order: V4 (COMPOSITION), V3 (TI_STATE STANDALONE), "
          "V2 (DIAGRAM), V1 (GENERATED_STILL)")
    print("No background_path or explicit source_path supplied anywhere -- V4 resolves "
          "V1/V2/V3's own outputs purely via source_beat_id.")
    print()
    ...
    v3_requirement = next(r for r in result.manifest.requirements if r.beat_id == "V3")
    v4_output_path = store.root / v4_asset.file_path

    print("Rendering order actually used (by manifest.assets order):")
    print(f"   {render_job_order}")
    print(f"   V1 rendered before V4 despite being authored last: "
          f"{render_job_order.index('V1') < render_job_order.index('V4')}")
    print(f"   V2 rendered before V4 despite being authored before it: "
          f"{render_job_order.index('V2') < render_job_order.index('V4')}")
    print()
    print("V3 (TI_STATE STANDALONE -- resolved as a composition overlay, never a "
          "RenderedVisualAsset of its own):")
    print(f"   status:            {v3_requirement.status.value}")
    print(f"   resolved_ti_state: {v3_requirement.resolved_ti_state.value}")
    print(f"   reference:         {v3_requirement.reference}")
    print()
    print("V4 (COMPOSITION -- final frame):")
    print(f"   output file: {v4_output_path}")
    print(f"   dimensions:  {v4_asset.width}x{v4_asset.height}")
    print(f"   notes:       {v4_asset.notes}")
    print()
    print(f"Total provider calls this run: {result.provider_call_count} (expected 1, only for V1)")
    print(f"Manifest preserves V1/V2/V4 as assets and V3 as a requirement: "
          f"assets={{a.beat_id for a in result.manifest.assets}} == "
          f"{{'V1', 'V2', 'V4'}} -> "
          f"{ {a.beat_id for a in result.manifest.assets} == {'V1', 'V2', 'V4'} }, "
          f"requirements={{r.beat_id for r in result.manifest.requirements}} == {{'V3'}} -> "
          f"{ {r.beat_id for r in result.manifest.requirements} == {'V3'} }")
    print()
    print(f"Human-inspectable final composite written to: {v4_output_path.resolve()}")
    return 0
