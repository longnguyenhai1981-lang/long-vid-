...
def _create_project_in_narrative(engine, idea=None, research_package=None):
    ...
    research_package = research_package or _valid_research_package(idea.central_question)
    ...
    return project_id, idea, research_package


def _ladder_node_dict(id="Q0", claim_ids=None, creates_next_question=None):
    return {
        "id": id,
        "question": f"Question {id}?",
        "why_viewer_cares": "It matters to the viewer",
        ...
        "information_gap": "What's still missing",
    }


def _plan_json(
    question_ladder=None,
    claim_ids_used=None,
    central_question="A question the model invented itself",
) -> str:
    return json.dumps(
        {
            "central_question": central_question,
            "scqa": {
                "situation": "A bridge opened to great fanfare",
                "complication": "It began to oscillate wildly in ordinary wind",
                "question": "Why would a well-engineered bridge do this?",
                "answer": "Self-excited aerodynamic flutter, not simple resonance",
            },
            "opening": "MYSTERY_FIRST",
            "question_ladder": question_ladder if question_ladder is not None else [_ladder_node_dict()],
            "ti_role": "Tí investigates the anomaly and reacts to each reveal",
            "physics_entry_points": ["When the flutter mechanism is revealed"],
            "deep_dive_points": [],
            "breath_moments": [],
            "ending": "Callback to the opening image of the bridge deck twisting",
            "claim_ids_used": claim_ids_used if claim_ids_used is not None else ["C001"],
        }
    )
...
def test_narrative_success(engine):
    project_id, idea, research_package = _create_project_in_narrative(engine)
    provider = FakeLLMProvider([_response(_plan_json())])

    narrative_engine = NarrativeEngine(engine, provider, _global_config(), _llm_settings())
    result = narrative_engine.run(NarrativeEngineInput(project_id=project_id))

    assert provider.call_count == 1
    assert isinstance(result.narrative, NarrativePlan)
    assert result.narrative.central_question == idea.central_question

    stored = get_artifact(engine, project_id, NARRATIVE_PLAN_ARTIFACT_TYPE, NarrativePlan)
    assert stored == result.narrative

    project = get_project(engine, project_id)
    assert project.narrative_plan_id == result.narrative.id
    assert project.state == ProjectState.NARRATIVE_REVIEW

    runs = list_module_runs_for_project(engine, project_id)
    assert len(runs) == 1
    assert runs[0].status == ModuleRunStatus.SUCCESS
    assert runs[0].output_id == str(result.narrative.id)
    assert result.business_correction_used is False


# ---------------------------------------------------------------------------
# Section 37: central question override
# ---------------------------------------------------------------------------


def test_narrative_central_question_always_overridden(engine):
    project_id, idea, research_package = _create_project_in_narrative(engine)
    provider = FakeLLMProvider(
        [_response(_plan_json(central_question="A totally different question"))]
    )
...
def test_narrative_structured_retry_then_success(engine):
    project_id, idea, research_package = _create_project_in_narrative(engine)
    invalid_json = '{"ti_role": "x"}'
    provider = FakeLLMProvider([_response(invalid_json), _response(_plan_json())])
...
def test_narrative_business_correction_then_success(engine):
    project_id, idea, research_package = _create_project_in_narrative(engine)
    bad = _response(
        _plan_json(question_ladder=[_ladder_node_dict("Q0", creates_next_question="Q999")])
    )
    good = _response(_plan_json())
    ...
    assert "unknown node id" in provider.received_requests[1].user_prompt
...
def test_narrative_business_correction_exhaustion(engine):
    project_id, idea, research_package = _create_project_in_narrative(engine)
    bad = _response(
        _plan_json(question_ladder=[_ladder_node_dict("Q0", creates_next_question="Q999")])
    )
    still_bad = _response(
        _plan_json(question_ladder=[_ladder_node_dict("Q0", creates_next_question="Q888")])
    )
    provider = FakeLLMProvider([bad, still_bad])

    narrative_engine = NarrativeEngine(engine, provider, _global_config(), _llm_settings())
    with pytest.raises(NarrativeBusinessValidationError):
        narrative_engine.run(NarrativeEngineInput(project_id=project_id))

    assert provider.call_count == 2

    project = get_project(engine, project_id)
    assert project.state == ProjectState.NARRATIVE
    assert project.narrative_plan_id is None
    ...
    with pytest.raises(ArtifactNotFoundError):
        get_artifact(engine, project_id, NARRATIVE_PLAN_ARTIFACT_TYPE, NarrativePlan)


# ---------------------------------------------------------------------------
# Section 50: invalid start state
# ---------------------------------------------------------------------------


def test_narrative_invalid_start_state(engine):
    project_id, idea, research_package = _create_project_in_narrative(engine)
    update_project_state(engine, project_id, ProjectState.R1_RESEARCH)
    provider = FakeLLMProvider([])
...
# ---------------------------------------------------------------------------
# Section 51: missing prior artifact
# ---------------------------------------------------------------------------


def test_narrative_missing_idea_candidate(engine):
    project_id = _new_project(engine)
    update_project_state(engine, project_id, ProjectState.IDEA_DISCOVERY)
    update_project_state(engine, project_id, ProjectState.IDEA_REVIEW)
    update_project_state(engine, project_id, ProjectState.R0_RESEARCH)
...
def test_narrative_missing_research_package(engine):
...
def test_narrative_llm_failure_marks_module_run_failed(engine):
    project_id, idea, research_package = _create_project_in_narrative(engine)
...
def test_narrative_structured_exhaustion(engine):
    project_id, idea, research_package = _create_project_in_narrative(engine)
    invalid_json = '{"ti_role": "x"}'
    provider = FakeLLMProvider(
