"""Filesystem storage for rendered visual asset bytes.

Visual asset bytes are never stored in SQLite (see app/storage/artifacts.py)
-- only a relative file path is ever recorded, in a VisualRenderManifest.
This store owns the one place that path is turned into bytes on disk.
Mirrors app/audio/storage.py's AudioFileStore exactly.
"""
...
def _validate_relative_path(relative_path: str) -> PurePosixPath:
    ...
    if not relative_path or not relative_path.strip():
        raise VisualPathError("relative_path cannot be blank")

    normalized = relative_path.replace("\\", "/")

    if normalized.startswith("/") or _DRIVE_LETTER_RE.match(relative_path):
        raise VisualPathError(f"relative_path must not be absolute: {relative_path!r}")

    pure_path = PurePosixPath(normalized)
    if ".." in pure_path.parts:
        raise VisualPathError(f"relative_path must not contain '..': {relative_path!r}")

    return pure_path


class VisualFileStore:
    ...
    def write(self, relative_path: str, data: bytes) -> Path:
        ...
        try:
            ...
            raise VisualWriteError(
                f"Failed to write visual asset file at relative path {relative_path!r}: {exc}"
            ) from exc
