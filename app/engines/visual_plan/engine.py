"""VisualPlanningEngine: converts the locked NarrativePlan/ScriptPlan/
VoicePlan into a visual representation strategy -- the tenth engine, and
the second in the production layer.

MVP_COMPLETE -> load+verify NarrativePlan/ScriptPlan/VoicePlan/
ResearchPackage -> verify VoicePlan freshness against the current
ScriptPlan (a real production-integrity gate, checked before any LLM call,
ModuleRun, or write) -> generate_structured(VisualPlan) -> business
validation (line coverage/order/adjacency, narrative-node alignment,
evidence-source integrity, C3 budget) -> one bounded correction attempt if
needed -> persist artifact -> persist ModuleRun.
...
any Project reference field (no visual_plan_id field exists on Project --
confirmed absent, not added this phase). It also never re-saves or
modifies ScriptPlan or VoicePlan. No image/video generation, no image
prompts, no storyboard rendering, no Timing/Assembly, no orchestrator, no
...
MODULE_NAME = "visual_plan_engine"
MODULE_VERSION = "0.1"
...
class VisualPlanningEngine:
    ...
    def run(self, engine_input: VisualPlanningInput) -> VisualPlanningResult:
        ...
        if project.state is not ProjectState.MVP_COMPLETE:
            raise EngineStateError(
                f"VisualPlanningEngine requires project state MVP_COMPLETE, got {project.state.value}"
            )
        ...
        try:
            visual_plan, generation_attempts, response, business_correction_used = (
                self._generate_visual_plan(
                    narrative_plan,
                    research_package,
                    script_plan,
                    voice_plan,
            ...
            self._artifact_repo.save_artifact(
                self._db_engine, engine_input.project_id, VISUAL_PLAN_ARTIFACT_TYPE, visual_plan
            )
            # No Project reference update: no visual_plan_id field exists on
            # Project (confirmed absent, Phase 14 compat finding 3).
        ...
        self._module_run_repo.save_module_run(
            self._db_engine, _with_success(run_record, visual_plan.id)
        )
    ...
    def _verify_voice_plan_is_fresh(self, project: Project, voice_plan: VoicePlan) -> None:
        ...
        write happens before this check. VoicePlan must have been generated
        for the ScriptPlan the project currently references."""
    ...
    def _generate_visual_plan(
        ...
        request = LLMRequest(
            system_prompt=build_system_prompt(
                self._global_config, narrative_plan, research_package, script_plan, voice_plan
            ),
        ...
        visual_plan = generation.value
        attempts = generation.attempts
        response = generation.response
        business_correction_used = False

        issues = validate_visual_plan(visual_plan, script_plan, narrative_plan, research_package)
        if issues:
            ...
            visual_plan = correction_generation.value
            attempts += correction_generation.attempts
            response = correction_generation.response

            issues = validate_visual_plan(visual_plan, script_plan, narrative_plan, research_package)
            if issues:
                raise VisualPlanBusinessValidationError(issues)

        return visual_plan, attempts, response, business_correction_used


def _with_success(run_record: ModuleRun, visual_plan_id) -> ModuleRun:
    return ModuleRun(
